...
const MEDAL_EMOJIS = ['\u{1F451}', '\u{1F948}', '\u{1F949}']; // crown, silver, bronze
const MEDAL_GLOWS = [
...
export function renderRankingScreen(
  ...
  drawGlassButton(ctx, allTabX, tabY, tabW, tabH, '전체', activeTab === 'all');

  const weeklyTabX = cx + tabGap / 2;
  drawGlassButton(ctx, weeklyTabX, tabY, tabW, tabH, '주간', activeTab === 'weekly');
  ...
  for (let i = 0; i < top10.length; i++) {
    ...
    if (isTop3) {
      ...
      drawGlassRect(ctx, listX, ry - 8, listW, rowH - 2, 10, glow.bg, glow.border);
      ...
      ctx.fillText(`${flag} ${entry.nickname}`, listX + 32, ry + 13);
      ...
      // 4위 이하
      if (entry.is_me) {
        drawGlassRect(ctx, listX, ry - 8, listW, rowH - 2, 8,
  ...
  if (myRank && myRank.rank > 10) {
    ...
    const myRowY = myY + 12;
    drawGlassRect(ctx, listX, myRowY - 8, listW, rowH - 2, 8,
      'rgba(255,107,53,0.15)', 'rgba(255,107,53,0.5)');
